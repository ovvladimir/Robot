...
class PCA9685:
    ...
    def setServo(self, channel, dutycycle):
        # Reverse
        if self.servos[channel].reverse is True:
            if self.servos[channel].signed is True:
                dutycycle = -dutycycle
            else:
...
class Servo:
    ...
    def set(self, signed, reverse, min_, max_, trim, exp):
        self.signed = signed
        ...
        self.servo_min = SERVO_MIN + SERVO_ZONE * (100.0 - self.min_) / 100.0
        self.servo_max = SERVO_MAX + SERVO_ZONE * (self.max_ - 100.0) / 100.0
        # Servo Trim
